...
run_command() {
    ...
    while [ \$success -eq 0 ]; do
        ...
        cp ../model3.par ../model3-\$sim_id.par

        # Conditionally add the extra line to the copied .par file
        if [ "\$add_line" -eq 1 ]; then
            echo "pop_event sweep \"sweep\" 1 U(0, 2000) .043 .5 1.0 1 U(0, 2000)" >> ../model3-\$sim_id.par
        fi

        # Run the command in the background
        env COSI_NEWSIM=1 COSI_MAXATTEMPTS=1000000 coalescent -p ../model3-\$sim_id.par -v -g --tped \${output_name} -n 1 -M -m -r 0 &
        local cmd_pid=\$!
